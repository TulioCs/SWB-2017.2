...
{
  /*
     Foi aplicado o teorema de De Morgan: "O complemento, ou negação de uma soma (OR)
     de variáveis é igual ao produto (AND) dos complementos das variáveis."
  */
  return (~x & ~y);
}
/* 
...
int bitsPares(int x) 
{
  ...
  /*
     Foi feita uma mascara de 8 bits com 01's e aplicado um (AND) entre o inteiro dado
     e a mascara para isso foi utilizados deslocamentos, feito essa operação
     todos os bits pares na palavra que são 1s foram mantidos e todos os impares
     ficaram 0s, sendo assim, se o numero atender a condição dada, volta
     a ser a mascara, com isso é feito um (XOR) do resultado com a propria mascara 
     e aplicado o operador diferente(!), o que garante que a resposta retornada é correta. 
...
int mascaraBit(int alto, int baixo) 
{
    ...
    /*
       É feita duas sub-mascaras a primeira aplicando deslocamento de alto+1 bits a esquerda
       de -1, a segunda aplicando deslocamento de baixo bits a esquerda de -1, 
       com isso delimitamos o intervalo a ser preenchido . Por fim, aplica 
       um xor entre as duas sub mascaras para obter a parte diferente e faz um (AND)
       com a segunda sub-mascara, para tratar do caso em que baixo > alto.

   */


  int masc = -1;
  int iInicial = (masc << alto) << 1; // pega o indice 
  
  int iFinal = masc << baixo;


  return (iInicial ^ iFinal) & iFinal;
...
{
  /*
  	Tendo em vista que a parte que nos indica se o numero pode ser representado ou não,
  	são os 16 bits menos signficativos, foi aplicado um deslocamento de 16 bits a esquerda,
  	e um deslocamento a 16 bits a direita, removendo assim a parte que não nos interessa,
  	depois disso é feito um (XOR) com intuito de saber se o número foi alterado ou não. 
  */
...
{
  /*
    Primeiro é aplicado duas vezes o operador (!) em x, se x for diferente de 0,
    o resu1tado é = 1 senão resultado = 0, feito isso, espalha o bit de 
    sinal com um deslocamento em x de 31 bits a direita, por fim é feito um | (OR)
    entre as duas operações, garantindo assim que a resposta resposta retornada é a desejada. 
  */
...
{
  /*
    Sabemos que o operador xor retorna 0 se os bits são iguais e
    1 se são diferentes, assim é realizado um xor entre x e y e aplicado
    o operador diferente duas vezes retornando o valor desejado.
...
{
  /*
    espalha o sinal com um deslocamento em x de 31 bits a direita 
    e faz um (AND) entre o deslocamento e  1.
  */
